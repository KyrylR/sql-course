...
*/
SELECT store_id,
       COUNT(CASE WHEN active = 1 THEN customer_id END) AS activve
FROM customer
GROUP BY store_id;
...
*/
SELECT COUNT(email) as count_of_all_cust_email
FROM customer;
...
SELECT MIN(replacement_cost) as least_expensive,
       ...
       AVG(replacement_cost) as average_rc
FROM film;
